(function() {
  ...
  var startApp = function(installId, successCB, eCB) {
    var errorCB = eCB || function() {};
        ...
        if (cfg) {
          if (cfg.startFile.contentType === "text/javascript") {
            ...
            childProc.on("exit", function(code,signal) {
              ...
              if (code !== 0 && code !== null && cfg.autostart === 1) {
                // ToDo - prevent thrashing - check for time between start and crash?
                console.log("restarting crashed service widget");
                setTimeout(function() {
                  startApp(installId);
                },30000);
              }
